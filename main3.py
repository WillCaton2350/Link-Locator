...
class parser:
        ...
        def main():
            try:
                ...
                print("""
                    ...
                    """)
                with open(f'subdomains3.csv', 'w') as file:
        ...
        def main1():
            try:
                time_altered.time_buffer(1)
                with open(f'subdomains4.csv', 'w') as file:
                    for url in urls.url_list: 
                        try:
                            value = urls.url_list[1]
                            response = requests.get(value)
                            response.raise_for_status()
                            soup = BeautifulSoup(response.content, 'html.parser')
                            for link in soup.findAll('a'):
        ...
        def main2():
            try:
                time_altered.time_buffer(1)
                with open(f'subdomains5.csv', 'w') as file:
                    for url in urls.url_list: 
                        try:
                            value = urls.url_list[2]
                            response = requests.get(value)
                            response.raise_for_status()
                            soup = BeautifulSoup(response.content, 'html.parser')
                            for link in soup.findAll('a'):
